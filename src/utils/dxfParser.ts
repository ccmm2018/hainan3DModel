 ...
 *
 * ──────────────────────────────────────────────────────────────────────
 * 能力边界（MVP 纯前端解析只保证以下能力）：
 *   ✓ 保证支持：
 *       · 标准 DXF（R12~R2018 的组码-值文本格式）
 *       · 直线房间：LWPOLYLINE / POLYLINE(VERTEX+SEQEND)，不含曲线段
 *       · 字段：TEXT / MTEXT
 *       · 编码：GBK / UTF-8 / UTF-16LE（自动探测）
 *       · 单文件 ≤ 20MB
 *   ✗ 不支持（命中即走后端解析服务兜底，预留接口 POST /api/cad/parse）：
 *       · 天正未转 T3 的私有实体（ACAD_XRECORD / 自定义图元等）
 *       · SPLINE / ELLIPSE 曲线房间（MVP 仅提取直线段，曲线段计入告警不静默丢弃）
 *       · 深层嵌套块（INSERT 嵌套 INSERT）
 *       · 单文件 > 20MB
 *       · 带洞多边形（柱洞 / 内凹）—— 数据结构已预留 holes 字段，后续版本支持
 * ──────────────────────────────────────────────────────────────────────
 *
